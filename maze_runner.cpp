...
bool walk(pos_t pos) {
	...
	while (!valid_positions.empty()) {
		...
		if (maze[pos.i][pos.j + 1] == 's' && pos.j < num_cols - 1) {
			exitFound = true;
		} else if (maze[pos.i][pos.j + 1] == 'x' && pos.j < num_cols - 1 ) {
			pos_t next_pos = {pos.i, pos.j + 1};
			valid_positions.push(next_pos);
		}
		
		// Verificação do caminho à esquerda
		if (maze[pos.i][pos.j - 1] == 's' && pos.j > 0 ) {
			exitFound = true;
		} else if (maze[pos.i][pos.j - 1] == 'x' && pos.j > 0 ) {
			maze[pos.i][pos.j] = 'o';
			pos_t next_pos = {pos.i, pos.j - 1};
			valid_positions.push(next_pos);
		}
		...
		if (maze[pos.i - 1][pos.j] == 's' && pos.i > 0 ) {
			exitFound = true;
		} else if (maze[pos.i - 1][pos.j] == 'x' && pos.i > 0  ) {
			pos_t next_pos = {pos.i - 1, pos.j};
			valid_positions.push(next_pos);
		}

		// Verificação do caminho abaixo
		if (maze[pos.i + 1][pos.j] == 's' && pos.i < num_rows - 1) {
			exitFound = true;
		} else if (maze[pos.i + 1][pos.j] == 'x' && pos.i < num_rows - 1) {
			pos_t next_pos = {pos.i + 1, pos.j};
			valid_positions.push(next_pos);
		}
